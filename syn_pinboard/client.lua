	...
	while true do
		...
		for k, v in pairs(Config.Posters) do
			if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.coords[1], v.coords[2], v.coords[3], false) < 1.0 then
				sleep       = false
				local label = CreateVarString(10, 'LITERAL_STRING', Config.Pinboard)
				...
				if active == false then
					if Citizen.InvokeNative(0xC92AC953F0A982AE, menubutton) then
						...
						TriggerServerEvent("syn_pinboard:get_posters", city)
						active = true
						Citizen.Wait(1000)
						Show_Poster(true)
					end
...
Citizen.CreateThread(function()
	for k, v in pairs(Config.Posters) do
		...
		Citizen.InvokeNative(0x9CB1A1623062F402, blips, v.blipname)
	end
end)
...
RegisterNUICallback('send_to_poster', function(data, cb)
	for k, v in pairs(Config.Posters) do
		...
		if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.coords[1], v.coords[2], v.coords[3], false) < 10.0 then
			if data.Title ~= "" then
				local s = data.Link
				TriggerEvent("vorp:TipBottom", Config.Message, 4000)
				TriggerServerEvent("syn_pinboard:set_link", data.Title, s:gsub("%'", ""), v.city, v.webhook)
				print(v.webhook)
				Citizen.Wait(1000)
